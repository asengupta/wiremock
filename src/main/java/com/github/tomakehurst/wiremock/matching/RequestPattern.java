...
public class RequestPattern {
  ...
  private int individualHashCode(Object o) {
    return new HashCodeBuilder().append(o).toHashCode();
  }

  public boolean isMatchedBy(Request request) {
    if (this.request != null) {
      HttpHeaders headers = request.getHeaders().without("Date").without("Host").without("Content-Length").without("Accept").without("Authorization").without("User-Agent").without("Connection").without("Content-Type");
      HttpHeaders thisHeaders = this.request.getHeaders().without("Date").without("Host").without("Content-Length").without("Accept").without("Authorization").without("User-Agent").without("Connection").without("Content-Type");
      String incomingBody = this.request.getBodyAsString().replaceAll("<messageID>[a-f0-9-]+</messageID>", "<messageID></messageID>").replaceAll("<sessionID>[a-f0-9-]+</sessionID>", "<sessionID></sessionID>");
      String cataloguedBody = request.getBodyAsString().replaceAll("<messageID>[a-f0-9-]+</messageID>", "<messageID></messageID>").replaceAll("<sessionID>[a-f0-9-]+</sessionID>", "<sessionID></sessionID>");
      boolean areBodiesEqual = incomingBody.equals(cataloguedBody);
      boolean areURLsEqual = this.request.getAbsoluteUrl().equals(request.getAbsoluteUrl());
      boolean areMethodsEqual = this.request.getMethod().value().equals(request.getMethod().value());
      boolean areHeadersEqual = thisHeaders.equals(headers);

      String lol = dotOrFail(areBodiesEqual) + dotOrFail(areURLsEqual) + dotOrFail(areMethodsEqual) + dotOrFail(areHeadersEqual);
      if ("f...".equals(lol)) {
        notifier().info(incomingBody);
        notifier().info("                              vs                                ");
        notifier().info(cataloguedBody);
      }
      notifier().info(lol);
      return areBodiesEqual
          && areURLsEqual
          && areMethodsEqual
          && areHeadersEqual;
    }
  ...
  private String dotOrFail(boolean areTheyEqual) {
    return areTheyEqual ? "." : "f";
  }
